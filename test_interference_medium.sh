...
for rate in "${RATES[@]}"; do
    ...
    run_test "Standard Container" "container_upload_targets.txt" $rate "in_c_medium_${rate}.txt"
    
    # Short cooldown after Standard test
    sleep 120
    
    # Test Kata Container  
    run_test "Kata Container" "kata_upload_targets.txt" $rate "in_k_medium_${rate}.txt"
...
for rate in "${RATES[@]}"; do
    echo "  in_c_medium_${rate}.txt - Standard container at ${rate} req/s"
    echo "  in_k_medium_${rate}.txt - Kata container at ${rate} req/s"
done
